...
def _package_exported_programs(
    ...
    for model_name, ep in exported_programs.items():
        ...
        archive_writer.write_string(
            WEIGHTS_CONFIG_FILENAME_FORMAT.format(model_name),
            json.dumps(_dataclass_to_dict(weights_config)),
        )

        constants_config = _package_constants(ep, archive_writer, pickle_protocol)
        archive_writer.write_string(
            CONSTANTS_CONFIG_FILENAME_FORMAT.format(model_name),
            json.dumps(_dataclass_to_dict(constants_config)),
        )
...
def _load_payload_config(
    ...
    return _dict_to_dataclass(
        schema.PayloadConfig,
        json.loads(archive_reader.read_string(filename_format.format(model_name))),
    )
...
def _load_state_dict(
    ...
    if legacy_weights_file in archive_reader.get_file_names():
        ...
        weights_config = _load_payload_config(
            archive_reader, model_name, WEIGHTS_CONFIG_FILENAME_FORMAT
        )
...
def _load_constants(
    ...
    if legacy_constants_file in archive_reader.get_file_names():
        logger.warning(
            ...
            "Please generate a new pt2 file by torch.export.save()."
        )
        return archive_reader.read_bytes(legacy_constants_file)
    else:
        constants_config = _load_payload_config(
            archive_reader, model_name, CONSTANTS_CONFIG_FILENAME_FORMAT
        )
