...
def forward(self, x):
    ...
    def test_triton_hop(self) -> None:
        ...
        self.assertEqual(kwargs[0].as_string, "add_kernel")
        self.assertEqual(kwargs[1].as_ints, [1, 1, 1])
        self.assertEqual(kwargs[2].as_ints, [2])

        self.assertEqual(len(triton_node.outputs), 1)
        self.assertIsNotNone(triton_node.outputs[0].as_tensors)
        self.assertEqual(len(triton_node.outputs[0].as_tensors), len(kwargs[2].as_ints))
