...
class TestMaxAutotune(TestCase):
    ...
    def test_max_autotune_contiguous_transform_non_contiguous_second_matrix(self, dynamic):
        """
        ...
        try:
            ...
            self.fail("Contiguous transform should not be used for contiguous matrices")
        ...
        with (
            ...
            compiled_func_non_contiguous = torch.compile(mm_non_contiguous, dynamic=dynamic)
            out2, code2 = run_and_get_code(compiled_func_non_contiguous, a, b_non_contiguous)
              
            # Should contain contiguous transform
            FileCheck().check("contiguous_mm").run(code2[0])

        # Verify correctness for both cases
        expected1 = mm_contiguous(a, b_contiguous)
        expected2 = mm_non_contiguous(a, b_non_contiguous)
        torch.testing.assert_close(out1, expected1, atol=1e-2, rtol=1e-2)
        torch.testing.assert_close(out2, expected2, atol=1e-2, rtol=1e-2)
